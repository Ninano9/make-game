        ...
        if (this.weapon.ammo > 0 && !this.weapon.isReloading && 
            ...
            if (this.weapon.type === 'SHOTGUN') {
                for (let i = 0; i < this.weapon.pellets; i++) {
                    let spread = (Math.random() - 0.5) * 0.4; // 산탄 확산
...
// 총알 클래스
class Bullet {
    constructor(x, y, angle, owner, damage = 25, range = 200) {
        this.x = x;
        this.y = y;
        this.startX = x;
        this.startY = y;
...
function createMuzzleFlash(x, y, angle) {
    for (let i = 0; i < 5; i++) {
        particles.push(new Particle(
            ...
            Math.random() * 6 + 2,
            {
                x: Math.cos(angle + (Math.random() - 0.5) * 0.5) * (Math.random() * 3 + 1),
                y: Math.sin(angle + (Math.random() - 0.5) * 0.5) * (Math.random() * 3 + 1)
            }
        ));
    }
}
